    ...
    if (req?.decoded?.user_id) {
      ...
      if (cartItem) {
        products.forEach((product) => {
          const cartProduct = cartItem.items.find(
      ...
      if (orders.length) {
        ...
        const filteredProducts = products.filter(
          (product) => !orderedProductIds.includes(product._id.toString())
        );
  ...
  try {
    ...
    if (!req.body.slashedPrice || req.body.slashedPrice <= 0) {
      return handleErrorMessages(
        res,
        "Slashed price must be a positive number"
      );
    }
    ...
    ["slashedPrice", "discount"].forEach(
      (key) => (req.body[key] = Number(req.body[key]))
    );
    // Calculate price dynamically based on slashedPrice and discount
    req.body.price =
      req.body.slashedPrice -
      (req.body.slashedPrice * (req.body.discount || 0)) / 100;
...
exports.updateProduct = async (req, res) => {
  try {
    ...
    if (
      req.body.slashedPrice !== undefined ||
      ...
      req.body.price = Math.floor(
        slashedPrice - (slashedPrice * discount) / 100
      );
    }
...
exports.uploadProductThumbnail = (req, res) => {
  ...
  uploadSingleFile(req, res, () => {
    ...
    return handleSuccessMessages(res, "Thumbnail uploaded successfully", {
      ...req.file,
      url: urlPath,
    });
  });
